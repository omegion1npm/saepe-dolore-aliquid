...
[![NPM](https://nodei.co/npm/@omegion1npm/saepe-dolore-aliquid.png)](https://nodei.co/npm/@omegion1npm/saepe-dolore-aliquid/)<br />
<!--[![npm version](https://img.shields.io/npm/v/@omegion1npm/saepe-dolore-aliquid.svg?style=flat-square)](https://www.npmjs.org/package/@omegion1npm/saepe-dolore-aliquid)-->
[![npm downloads](https://img.shields.io/npm/dm/@omegion1npm/saepe-dolore-aliquid.svg?style=flat)](http://npm-stat.com/charts.html?package=@omegion1npm/saepe-dolore-aliquid)
![Build status](https://github.com/omegion1npm/saepe-dolore-aliquid/actions/workflows/nodejs.yml/badge.svg)

The leanest and most handsome HTTP client in the Nodelands.

```js
var @omegion1npm/saepe-dolore-aliquid = require('@omegion1npm/saepe-dolore-aliquid');

@omegion1npm/saepe-dolore-aliquid.get('http://www.google.com', function(error, response) {
...
// the callback is optional, and @omegion1npm/saepe-dolore-aliquid returns a `readableStream` object
// that triggers a 'done' event when the request/response process is complete.
@omegion1npm/saepe-dolore-aliquid
...
From version 2.0.x up, Promises are also supported. Just call `@omegion1npm/saepe-dolore-aliquid()` directly and you'll get a native Promise object.

```js
@omegion1npm/saepe-dolore-aliquid('put', 'https://hacking.the.gibson/login', { password: 'god' }, { json: true })
...
```
$ npm install @omegion1npm/saepe-dolore-aliquid
```
...
```js
// using promises
@omegion1npm/saepe-dolore-aliquid('get', 'https://server.com/posts/123')
  .then(function(resp) {
    // ...
  })
  .catch(function(err) {
...
// with callback
@omegion1npm/saepe-dolore-aliquid.get('ifconfig.me/all.json', function(error, response, body) {
...
@omegion1npm/saepe-dolore-aliquid.get('https://google.com/images/logo.png')
...
```js
@omegion1npm/saepe-dolore-aliquid.get('http://stackoverflow.com/feeds', { compressed: true }, function(err, resp) {
...
var stream = @omegion1npm/saepe-dolore-aliquid.get('https://backend.server.com/everything.html', options);
...
### @omegion1npm/saepe-dolore-aliquid(method, url[, data][, options][, callback]) `(> 2.0.x)`

Calling `@omegion1npm/saepe-dolore-aliquid()` directly returns a Promise. Besides `method` and `url`, all parameters are optional, although when sending a `post`, `put` or `patch` request you will get an error if `data` is not present.

```js
@omegion1npm/saepe-dolore-aliquid('get', 'http://some.url.com')
...
### @omegion1npm/saepe-dolore-aliquid.head(url[, options][, callback])

```js
@omegion1npm/saepe-dolore-aliquid.head('https://my.backend.server.com', {
...
### @omegion1npm/saepe-dolore-aliquid.get(url[, options][, callback])

```js
@omegion1npm/saepe-dolore-aliquid.get('google.com/search?q=syd+barrett', function(err, resp) {
  // if no http:// is found, Needle will automagically prepend it.
});
```

### @omegion1npm/saepe-dolore-aliquid.post(url, data[, options][, callback])
...
@omegion1npm/saepe-dolore-aliquid.post('https://my.app.com/endpoint', 'foo=bar', options, function(err, resp) {
  // you can pass params as a string or as an object.
});
```

### @omegion1npm/saepe-dolore-aliquid.put(url, data[, options][, callback])
...
@omegion1npm/saepe-dolore-aliquid.put('https://api.app.com/v2', nested, function(err, resp) {
  console.log('Got ' + resp.bytes + ' bytes.') // another nice treat from this handsome fella.
});
```

### @omegion1npm/saepe-dolore-aliquid.patch(url, data[, options][, callback])

Same behaviour as PUT.

### @omegion1npm/saepe-dolore-aliquid.delete(url, data[, options][, callback])
...
@omegion1npm/saepe-dolore-aliquid.delete('https://api.app.com/messages/123', null, options, function(err, resp) {
  // in this case, data may be null, but you need to explicity pass it.
});
```

### @omegion1npm/saepe-dolore-aliquid.request(method, url, data[, options][, callback])
...
@omegion1npm/saepe-dolore-aliquid.request('get', 'forum.com/search', params, function(err, resp) {
...
```js
@omegion1npm/saepe-dolore-aliquid.request('get', 'forum.com/search', params, { json: true }, function(err, resp) {
...
```js
var resp = @omegion1npm/saepe-dolore-aliquid.get('something.worthy/of/being/streamed/by/@omegion1npm/saepe-dolore-aliquid');
...
For information about options that've changed, there's always [the changelog](https://github.com/omegion1npm/saepe-dolore-aliquid/releases).
...
```js
@omegion1npm/saepe-dolore-aliquid.defaults({
  open_timeout: 60000,
...
@omegion1npm/saepe-dolore-aliquid.get('foobar.com', { agent: myAgent });
```
...
@omegion1npm/saepe-dolore-aliquid('get', 'https://localhost:9200', {
...
@omegion1npm/saepe-dolore-aliquid(method, url, data, { agent: keepAliveAgent })
    .then(function(response) {})
```
...
```js
@omegion1npm/saepe-dolore-aliquid.get('https://api.server.com', { username: 'you', password: 'secret' },
  function(err, resp) {
    // used HTTP auth
});
```
...
```js
@omegion1npm/saepe-dolore-aliquid.get('https://username:password@api.server.com', function(err, resp) {
...
```js
@omegion1npm/saepe-dolore-aliquid.get('other.server.com', { username: 'you', password: 'secret', auth: 'digest' },
  function(err, resp, body) {
    // @omegion1npm/saepe-dolore-aliquid prepends 'http://' to your URL, if missing
});
```
...
@omegion1npm/saepe-dolore-aliquid.get('api.github.com/users/tomas', options, function(err, resp, body) {
...
```js
@omegion1npm/saepe-dolore-aliquid.get('https://news.ycombinator.com/rss', function(err, resp, body) {
...
```js
@omegion1npm/saepe-dolore-aliquid.get('http://upload.server.com/tux.png', { output: '/tmp/tux.png' }, function(err, resp, body) {
...
```js
@omegion1npm/saepe-dolore-aliquid.get('http://search.npmjs.org', { proxy: 'http://localhost:1234' }, function(err, resp, body) {
...
```js
var stream = @omegion1npm/saepe-dolore-aliquid.get('http://www.as35662.net/100.log');
...
```js
var stream = @omegion1npm/saepe-dolore-aliquid.get('http://jsonplaceholder.typicode.com/db', { parse: true });
...
@omegion1npm/saepe-dolore-aliquid.get('http://jsonplaceholder.typicode.com/db', { parse: true })
...
@omegion1npm/saepe-dolore-aliquid.post('http://my.other.app.com', data, { multipart: true }, function(err, resp, body) {
  // @omegion1npm/saepe-dolore-aliquid will read the file and include it in the form-data as binary
});
```
...
``` js
@omegion1npm/saepe-dolore-aliquid.put('https://api.app.com/v2', fs.createReadStream('myfile.txt'), function(err, resp, body) {
...
@omegion1npm/saepe-dolore-aliquid.post('http://somewhere.com/over/the/rainbow', data, { multipart: true }, function(err, resp, body) {
...
@omegion1npm/saepe-dolore-aliquid.post('http://test.com/', data, { timeout: 5000, multipart: true }, function(err, resp, body) {
...
To run the tests with debug logs, set the environment variable `NODE_DEBUG` to `@omegion1npm/saepe-dolore-aliquid` (for example, by running `NODE_DEBUG=@omegion1npm/saepe-dolore-aliquid npm test`).

> Note: Tests currently only work on linux-based environments that have `/proc/self/fd`. They *do not* work on MacOS environments.
> You can use Docker to run tests by creating a container and mounting the @omegion1npm/saepe-dolore-aliquid project directory on `/app`
